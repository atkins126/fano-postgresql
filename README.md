# Fano PostgreSql Database

Sample Fano Framework web application that demonstrate how to load data from PostgreSql database.
...
```
$ git clone git@github.com:fanofamework/fano-postgresql.git --recursive
...
```
$ sudo fanocli --deploy-scgi=postgresql.fano
```
...
Open Internet browser and go to URL http://postgresql.fano.

## Database setup

Make sure you have working PostgreSQL installation.

From command line, create PostgreSQL user named `fanopsql`

```
$ sudo -u postgres createuser fanopqsql
$ sudo adduser fanopqsql
```

Create database with same name as user

```
$ sudo -u postgres createdb -O fanopqsql fanopqsql
```
Now we need to create table named `tb_users`

```
$ sudo -u fanopqsql psql
```
If you have create database and user correctly, it should display prompt as follow,

```
fanopqsql=>
```

Execute following SQL command to create table
...
```
INSERT INTO TABLE tb_users (userId, username, password) VALUES (1, 'joko', MD5('joko'));
```
...
<VirtualHost *:80>
    ...
    ServerAdmin admin@postgresql.fano
    DocumentRoot "/path/to/repo/directory/public"

    ServerName postgresql.fano
    ServerAlias *.postgresql.fano

    ErrorLog /var/log/apache2/postgresql.fano-error.log
    CustomLog /var/log/apache2/postgresql.fano-access.log combined
